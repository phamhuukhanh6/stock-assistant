function buildPrompt({ messages, systemPrompt, dataContext, isStockQuery, isComparison }) {
  ...
  // Quy tắc cốt lõi từ Gemini.md & Tối ưu Quota
  const coreRules = [
  ...
  // Aggressive context trimming: Max 1500 chars total for data
  const safeDataContext = dataContext.length > 1500 ? dataContext.slice(0, 1500) + '...[Dữ liệu đã được tối ưu]' : dataContext;

  if (isStockQuery) {
    if (isComparison) {
      prompt += `\nNHIỆM VỤ: SO SÁNH CÁC MÃ CHỨNG KHOÁN (PHONG CÁCH CHUYÊN GIA).
YÊU CẦU:
- Lập bảng so sánh chỉ số tài chính cơ bản (P/E, P/B, ROE, Vốn hóa).
- Giải thích các chỉ số này cho F0.
- Đánh giá ưu/nhược điểm từng mã dựa trên bối cảnh ngành.
- Kết luận mã nào có sức khỏe tài chính tốt hơn.
\n`;
    } else {
      prompt += `\nNHIỆM VỤ: PHÂN TÍCH CHI TIẾT MÃ CHỨNG KHOÁN.
YÊU CẦU:
- Đánh giá tổng quan: [TÍCH CỰC / THEO DÕI / TIÊU CỰC].
- Phân tích Tài chính & Kỹ thuật bằng cấu trúc đơn giản.
- Kết nối thông tin vĩ mô/ngành với doanh nghiệp.
\n`;
    }
...
function truncateMessages(messages, limit = 10) {
  if (messages.length <= limit) return messages;
  return messages.slice(-limit);
}

module.exports = { buildPrompt, truncateMessages };
